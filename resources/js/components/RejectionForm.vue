  ...
  <form @submit.prevent="submit" class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div class="flex items-center space-x-2">
        <Checkbox v-model="form.disputed" id="disputed" />
        <Label for="disputed">Disputed</Label>
      </div>
    </div>

    <div
      class="flex-col space-y-2 sm:space-y-0 sm:flex-row sm:justify-end sm:space-x-2 pt-4 border-t flex"
    >
      <Button
        type="button"
        @click="emit('close')"
...
const form = useForm({
  ...
  disputed:
    props.rejection && props.rejection.disputed !== null
      ? parseInt(props.rejection.disputed) === 1
      : false,
  driver_controllable:
    props.rejection && props.rejection.driver_controllable !== null
    ...
    if (newVal) {
      ...
      form.driver_controllable =
        newVal.driver_controllable !== null
          ? parseInt(newVal.driver_controllable) === 1
          : null;
    } else {
      form.reset();
    }
