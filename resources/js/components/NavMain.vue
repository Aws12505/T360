...
const isActive = (itemHref: string) => {
  const hrefPath = new URL(itemHref, window.location.origin).pathname
  const current = currentPath.value

  // Drivers should only match exact
  if (hrefPath.endsWith('/drivers')) {
...
<template>
  <SidebarGroup class="px-1 py-0">
    <SidebarGroupLabel class="text-[color:hsl(var(--fixing-sidebar-foreground))] !important">Platform
    </SidebarGroupLabel>
    <SidebarMenu>
