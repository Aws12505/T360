        ...
        <div
            ...
            <div class="mb-2 flex flex-col items-center justify-between px-2 sm:flex-row md:mb-4 lg:mb-6">
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <!-- responsive here -->
                    <Button v-if="selectedRepairOrders.length > 0" @click="confirmDeleteSelected()"
                        variant="destructive">
                        <!-- responsive here -->
                    ...
                    <!-- responsive here -->
                    <Button class="px-2 py-0 md:px-4 md:py-2" v-if="SuperAdmin" @click="openAreasOfConcernModal"
                        variant="outline">
                        <!-- responsive here -->
                    ...
                    <!-- responsive here -->
                    <Button class="px-2 py-0 md:px-4 md:py-2" v-if="SuperAdmin" @click="openVendorsModal"
                        variant="outline">
                        <!-- responsive here -->
                        <Icon name="settings" class="mr-1 h-4 w-4 md:mr-2" />
                        Manage Vendors
                    ...
                    <!-- responsive here -->
                    <Button class="px-2 py-0 md:px-4 md:py-2" v-if="SuperAdmin" @click="openWoStatusesModal"
                        variant="outline">
                        <!-- responsive here -->
                        <Icon name="settings" class="mr-1 h-4 w-4 md:mr-2" />
                        Manage WO Statuses
                    ...
                    <div class="relative">
                        <!-- responsive here -->
                        <Button class="px-2 py-0 md:px-4 md:py-2" @click="showUploadOptions = !showUploadOptions"
                            variant="secondary">
                            <!-- responsive here -->
                        ...
                        <div v-if="showUploadOptions"
                            class="absolute right-0 z-10 mt-1 w-48 rounded-md border bg-background shadow-lg">
                            <div class="py-1">
                                ...
                                <a :href="templateUrl" download="Repair Orders Template.csv"
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            ...
                            <div class="mt-1 text-sm text-red-700 dark:text-red-200">
                                {{ props.canceledQSInvoices?.length || 0 }} invoices found with canceled WO status but
                                still on QS. These need
                                immediate attention.
                            </div>
                            <div class="mt-2">
                ...
                <CardContent class="p-2 md:p-4 lg:p-6">
                    ...
                    <div class="flex flex-col items-center gap-2 md:items-start">
                        ...
                        <div v-if="dateRange" class="text-sm text-muted-foreground">
                            <span v-if="activeTab === 'yesterday' && dateRange.start"> Showing data from {{
                                formatDate(dateRange.start) }} </span>
            ...
            <div class="mx-auto mb-6 grid max-w-[95vw] grid-cols-1 gap-4 md:max-w-[64vw] md:grid-cols-2 lg:max-w-full"
                ...
                <div class="rounded-lg border bg-card shadow-sm">
                    ...
                    <div class="p-4">
                        <ul class="space-y-3">
                            ...
                            <li v-for="(area, index) in topAreasOfConcern" :key="index"
                                class="flex items-center justify-between pr-4">
                                <span class="text-sm">{{ area.concern }}</span>
                                <Badge variant="outline">{{ area.count }}</Badge>
                            </li>
                            <li v-if="topAreasOfConcern.length === 0" class="text-center text-muted-foreground">No data
                                available</li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="rounded-lg border bg-card shadow-sm">
                    ...
                    <div class="p-4">
                        <ul class="space-y-3">
                            ...
                            <li v-for="(truck, index) in topWorkOrdersByTruck" :key="index"
                                class="flex items-center justify-between pr-4">
                                <span class="text-sm">{{ truck.truckid }}</span>
                                <Badge variant="outline">{{ truck.work_order_count }}</Badge>
                            </li>
                            <li v-if="topWorkOrdersByTruck.length === 0" class="text-center text-muted-foreground">No
                                data available</li>
                        </ul>
                    </div>
                </div>
            ...
            <div class="mx-auto mb-6 max-w-[95vw] overflow-x-auto rounded-lg border bg-card p-4 shadow-sm md:max-w-[64vw] lg:max-w-full"
                v-if="!SuperAdmin">
                    ...
                    <div>
                        ...
                        <Popover v-model:open="outstandingDateOpen">
                            <PopoverTrigger as-child>
                                <Button id="outstanding-since" variant="outline"
            ...
            <div v-if="outstandingInvoices && outstandingInvoices.length > 0"
                ...
                <div class="flex items-center justify-between border-b p-4">
                    ...
                    <div class="flex items-start justify-between">
                        ...
                        <Button variant="ghost" size="sm"
                            @click="showOutstandingInvoicesSection = !showOutstandingInvoicesSection" class="ml-4 mt-1">
                            {{ showOutstandingInvoicesSection ? 'Hide Invoices' : 'Show Invoices' }}
                            <Icon :name="showOutstandingInvoicesSection ? 'chevron-up' : 'chevron-down'"
                                class="ml-2 h-4 w-4" />
                        </Button>
                    </div>
                </div>
                <div v-if="showOutstandingInvoicesSection" class="mb-6 rounded-lg border bg-card shadow-sm">
                    ...
                    <div class="overflow-x-auto">
                        ...
                        <div
                            class="/* ~15rem on xs */ /* ~20rem on sm+ */ /* ~24rem on md+ */ max-h-60 overflow-y-auto sm:max-h-80 md:max-h-96">
                            <Table class="min-w-full">
                                ...
                                <TableBody>
                                    <TableRow v-for="invoice in outstandingInvoices" :key="invoice.ro_number">
                                        ...
                                        <TableCell class="text-right">{{ formatCurrency(invoice.invoice_amount) }}
                                        </TableCell>
                                    </TableRow>
                                </TableBody>
                            </Table>
            ...
            <div v-else-if="showOutstandingInvoicesSection" class="mb-6 rounded-lg border bg-card shadow-sm">
                ...
                <div class="p-4 text-center text-muted-foreground">No outstanding invoices match the current criteria
                </div>
            </div>
            <!-- Filters Section -->
            <Card>
                ...
                <CardHeader class="p-2 md:p-4 lg:p-6">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <div v-if="!showFilters && (filters.search || filters.vendor_id || filters.status)"
                                class="ml-4 flex flex-wrap gap-2 text-xs text-muted-foreground">
                                <span v-if="filters.search" class="rounded-full bg-muted px-2 py-1">Search: {{
                                    filters.search }}</span>
                                <span v-if="filters.vendor_id" class="rounded-full bg-muted px-2 py-1">Vendor: {{
                                    vendors.find((v) => v.id == filters.vendor_id)?.vendor_name || filters.vendor_id
                                    }}</span>
                                <span v-if="filters.status_id" class="rounded-full bg-muted px-2 py-1">
                                    Status: {{woStatuses.find((s) => s.id == filters.status_id)?.name ||
                                        filters.status_id}}
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="flex flex-col gap-1 md:gap-4">
                        ...
                        <div class="grid w-full grid-cols-1 gap-1 sm:grid-cols-3 md:gap-4">
                            <div>
                                ...
                                <!-- responsive here -->
                                <Input class="h-9 px-1 py-1 md:px-2 md:py-1 lg:h-10 lg:px-3 lg:py-2" id="search"
                                    v-model="filters.search" type="text" placeholder="Search by RO#, Invoice, WO#..." />
                            </div>
                            <div>
                                <Label for="vendor_filter">Vendor</Label>
                                <select id="vendor_filter" v-model="filters.vendor_id"
                            ...
                            <div>
                                ...
                                <select id="status_filter" v-model="filters.status_id"
                                    class="flex h-10 w-full items-center rounded-md border border-input bg-background px-3 py-2 text-sm">
            ...
            <div v-if="!hasData" class="flex flex-col items-center justify-center rounded-lg border bg-muted/20 py-16">
                ...
                <h2 class="text-center text-2xl font-bold text-muted-foreground">There is No Data to give Information
                    about.</h2>
            </div>
            <!-- responsive here -->
                ...
                <CardContent class="p-0">
                        ...
                        <Table class="relative h-[600px] overflow-auto">
                            <TableHeader>
                                <TableRow class="sticky top-0 z-10 border-b bg-background hover:bg-background">
                                    ...
                                    <TableHead class="cursor-pointer whitespace-nowrap" @click="sortBy('ro_number')">
                                        <div class="flex items-center">
                                            RO#
                                            <SortIndicator :column="'ro_number'" :sortColumn="sortColumn"
                                    ...
                                    <TableHead class="cursor-pointer whitespace-nowrap" @click="sortBy('ro_open_date')">
                                        <div class="flex items-center">
                                            Open Date
                                            <SortIndicator :column="'ro_open_date'" :sortColumn="sortColumn"
                                                :sortDirection="sortDirection" />
                                        </div>
                                    </TableHead>
                                    <TableHead class="cursor-pointer whitespace-nowrap"
                                        @click="sortBy('ro_close_date')">
                                        <div class="flex items-center">
                                            Close Date
                                            <SortIndicator :column="'ro_close_date'" :sortColumn="sortColumn"
                                                :sortDirection="sortDirection" />
                                        </div>
                                    </TableHead>
                            ...
                            <TableBody>
                                <TableRow v-if="repairOrders.data.length === 0">
                                    <TableCell :colspan="SuperAdmin ? 16 : 15" class="py-8 text-center"> No repair
                                        orders found. </TableCell>
                                </TableRow>
                                <TableRow v-for="order in repairOrders.data" :key="order.id" class="hover:bg-muted/50">
                                    ...
                                    <TableCell class="text-center">
                                        <input type="checkbox" :value="order.id" v-model="selectedRepairOrders"
                                            class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
                                    </TableCell>
                                    <TableCell class="whitespace-nowrap">{{ order.ro_number }}</TableCell>
                                    <!-- Add Tenant cell for SuperAdmin -->
                                    <TableCell v-if="SuperAdmin" class="whitespace-nowrap">{{ order.tenant?.name ?? '—'
                                        }}</TableCell>
                                    <TableCell class="whitespace-nowrap">{{ formatDate(order.ro_open_date) }}
                                    </TableCell>
                                    ...
                                    <TableCell class="whitespace-nowrap">
                                        ...
                                        <span v-if="order.vendor?.deleted_at"
                                            class="ml-1 text-xs text-red-500">(Deleted)</span>
                                    </TableCell>
                                    <TableCell class="whitespace-nowrap">
                                        <div v-if="order.areas_of_concern && order.areas_of_concern.length > 0">
                                            <span v-for="(area, index) in order.areas_of_concern" :key="area.id"
                                                class="inline-block">
                                                {{ area.concern }}
                                                <span v-if="area.deleted_at"
                                                    class="text-xs text-red-500">(Deleted)</span>
                                    ...
                                    <TableCell class="whitespace-nowrap">{{ formatCurrency(order.invoice_amount) }}
                                    </TableCell>
                                    <TableCell class="whitespace-nowrap">{{ order.invoice_received ? 'Yes' : 'No' }}
                                    </TableCell>
                                    <TableCell class="whitespace-nowrap">{{
                    ...
                    <div class="border-t bg-muted/20 px-4 py-3" v-if="repairOrders.links">
                        ...
                        <div class="flex flex-col items-center justify-between gap-2 sm:flex-row">
                            <div class="text-sm text-muted-foreground">Showing {{ repairOrders.data.length }} entries
                            </div>
                            <!-- responsive here -->
                            <div class="flex w-full flex-col items-center gap-2 sm:w-auto sm:flex-row sm:gap-4">
                                ...
                                <div class="flex flex-wrap">
                                    <Button v-for="link in repairOrders.links" :key="link.label"
                                        @click="visitPage(link.url)" :disabled="!link.url" variant="ghost" size="sm"
                                        class="mx-1"
                    ...
                    <form @submit.prevent="submitForm"
                        ...
                        <div v-if="SuperAdmin" class="col-span-2">
                            ...
                            <div class="relative mt-1">
                                ...
                                <div
                                    ...
                                    <svg class="h-3 w-3 opacity-50 sm:h-4 sm:w-4" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <Popover v-model:open="roOpenDateOpen">
                                <PopoverTrigger as-child>
                                    <Button id="ro_open_date" variant="outline"
                                        class="mt-1 h-9 w-full justify-start px-3 py-1 text-left text-xs font-normal sm:h-10 sm:text-sm">
                                        <CalendarIcon class="mr-2 h-4 w-4" />
                                        {{
                                            roOpenDatePicker
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <Popover v-model:open="roCloseDateOpen">
                                <PopoverTrigger as-child>
                                    <Button id="ro_close_date" variant="outline"
                                        class="mt-1 h-9 w-full justify-start px-3 py-1 text-left text-xs font-normal sm:h-10 sm:text-sm">
                                        <CalendarIcon class="mr-2 h-4 w-4" />
                                        {{
                                            roCloseDatePicker
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <div class="relative mt-1">
                                <select id="truck_id" v-model="form.truck_id" required
                                    class="flex h-9 w-full appearance-none items-center rounded-md border border-input bg-background px-3 py-1 text-xs ring-offset-background focus:outline-none focus:ring-1 focus:ring-ring focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 sm:h-10 sm:text-sm">
                            ...
                            <div class="mt-1 rounded-md border border-input bg-background p-2">
                                <div class="mb-2 flex flex-wrap gap-1">
                                    <div v-for="selectedId in form.area_of_concerns" :key="selectedId"
                                        ...
                                        <span v-if="areasOfConcern.find((a) => a.id === selectedId)?.deleted_at"
                                            class="ml-1 text-[0.65rem] text-red-500 sm:text-xs">(Deleted)</span>
                                        <button type="button" @click="removeAreaOfConcern(selectedId)"
                                ...
                                <div class="relative">
                                    <select id="area_of_concerns_select" @change="addAreaOfConcern($event)"
                                        class="flex h-9 w-full appearance-none items-center rounded-md border border-input bg-background px-3 py-1 text-xs ring-offset-background focus:outline-none focus:ring-1 focus:ring-ring focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 sm:h-10 sm:text-sm">
                                        <option value="">Select an area of concern to add</option>
                                        <option v-for="area in availableAreasOfConcern" :key="area.id" :value="area.id"
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea id="repairs_made" v-model="form.repairs_made"
                                class="mt-1 flex min-h-[60px] w-full rounded-md border border-input bg-background px-3 py-1.5 text-xs ring-offset-background focus:outline-none focus:ring-1 focus:ring-ring focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 sm:min-h-[80px] sm:text-sm"></textarea>
                        </div>
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <div class="relative mt-1">
                                <select id="vendor_id" v-model="form.vendor_id" required
                                    class="flex h-9 w-full appearance-none items-center rounded-md border border-input bg-background px-3 py-1 text-xs ring-offset-background focus:outline-none focus:ring-1 focus:ring-ring focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 sm:h-10 sm:text-sm">
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <div class="relative mt-1">
                                <select id="wo_status_id" v-model="form.wo_status_id" required
                                    class="flex h-9 w-full appearance-none items-center rounded-md border border-input bg-background px-3 py-1 text-xs ring-offset-background focus:outline-none focus:ring-1 focus:ring-ring focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 sm:h-10 sm:text-sm">
                                    <option disabled value="">Select status</option>
                                    <option v-for="status in woStatuses" :key="status.id" :value="status.id"
                                        :disabled="status.deleted_at">
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <div class="mt-1 flex items-center space-x-2">
                                <input id="invoice_received" v-model="form.invoice_received" type="checkbox"
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <div class="relative mt-1">
                                <select id="on_qs" v-model="form.on_qs" required
                                    class="flex h-9 w-full appearance-none items-center rounded-md border border-input bg-background px-3 py-1 text-xs ring-offset-background focus:outline-none focus:ring-1 focus:ring-ring focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 sm:h-10 sm:text-sm">
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <Popover v-model:open="qsInvoiceDateOpen">
                                <PopoverTrigger as-child>
                                    <Button id="qs_invoice_date" variant="outline"
                                        class="mt-1 h-9 w-full justify-start px-3 py-1 text-left text-xs font-normal sm:h-10 sm:text-sm">
                                        <CalendarIcon class="mr-2 h-4 w-4" />
                                        {{
                                            qsInvoiceDatePicker
                        ...
                        <div v-if="formAction === 'Update'">
                            ...
                            <div class="relative mt-1">
                                <select id="disputed" v-model="form.disputed" required
                                    class="flex h-9 w-full appearance-none items-center rounded-md border border-input bg-background px-3 py-1 text-xs ring-offset-background focus:outline-none focus:ring-1 focus:ring-ring focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 sm:h-10 sm:text-sm">
                        ...
                        <div class="col-span-2" v-if="formAction === 'Update'">
                            ...
                            <textarea id="dispute_outcome" v-model="form.dispute_outcome"
                                class="mt-1 flex min-h-[60px] w-full rounded-md border border-input bg-background px-3 py-1.5 text-xs ring-offset-background focus:outline-none focus:ring-1 focus:ring-ring focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 sm:min-h-[80px] sm:text-sm"></textarea>
                        </div>
                        ...
                        <DialogFooter class="col-span-2 mt-3 flex flex-col gap-2 sm:flex-row sm:gap-3">
                            <Button type="button" @click="closeModal" variant="outline"
                                class="h-9 px-4 py-1 text-xs sm:h-10 sm:text-sm">Cancel</Button>
                ...
                <DialogContent class="max-w-[95vw] sm:max-w-[425px]">
                    ...
                    <DialogFooter class="flex-col gap-2 sm:flex-row">
                        ...
                        <Button type="button" @click="confirmDelete" variant="destructive"
                            class="w-full sm:w-auto">Delete</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog v-model:open="showAreasOfConcernModal">
                <DialogContent class="max-w-[95vw] sm:max-w-[90vw] md:max-w-[600px]">
                    <DialogHeader>
                        ...
                        <DialogDescription class="text-sm sm:text-base"> Add or remove areas of concern for repair
                            orders. </DialogDescription>
                    </DialogHeader>
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        <TableRow v-if="areasOfConcern.length === 0">
                                            <TableCell colspan="2" class="py-4 text-center text-sm">No areas of concern
                                                found.</TableCell>
                                        </TableRow>
                                        <TableRow v-for="area in areasOfConcern" :key="area.id">
                                            ...
                                            <TableCell>
                                                <div class="flex space-x-1 sm:space-x-2">
                                                    <Button v-if="area.deleted_at"
                                                        @click="restoreAreaOfConcern(area.id)" variant="outline"
                                                        size="sm">
                                                        <Icon name="undo" class="h-4 w-4" />
                                                    </Button>
                                                    <Button v-if="!area.deleted_at"
                                                        @click="deleteAreaOfConcern(area.id)" variant="destructive"
                                                        size="sm">
                                                        <Icon name="trash" class="h-4 w-4" />
                                                    </Button>
                                                    <Button v-if="area.deleted_at"
                                                        @click="forceDeleteAreaOfConcern(area.id)" variant="destructive"
                                                        size="sm">
                    ...
                    <DialogFooter>
                        <Button @click="showAreasOfConcernModal = false" variant="outline"
                            class="w-full sm:w-auto">Close</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog v-model:open="showVendorsModal">
                <DialogContent class="max-w-[95vw] sm:max-w-[90vw] md:max-w-[600px]">
                    <DialogHeader>
                        ...
                        <DialogDescription class="text-sm sm:text-base"> Add or remove vendors for repair orders.
                        </DialogDescription>
                    </DialogHeader>
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        <TableRow v-if="vendors.length === 0">
                                            <TableCell colspan="2" class="py-4 text-center text-sm">No vendors found.
                                            </TableCell>
                                        </TableRow>
                                        <TableRow v-for="vendor in vendors" :key="vendor.id">
                                            <TableCell class="text-xs sm:text-sm">
                                                {{ vendor.vendor_name }}
                                                <span v-if="vendor.deleted_at"
                                                    class="ml-1 text-xs text-red-500">(Deleted)</span>
                                            </TableCell>
                                            <TableCell>
                                                <div class="flex space-x-1 sm:space-x-2">
                                                    <Button v-if="vendor.deleted_at" @click="restoreVendor(vendor.id)"
                                                        variant="outline" size="sm">
                                                        <Icon name="undo" class="h-4 w-4" />
                                                    </Button>
                                                    <Button v-if="!vendor.deleted_at" @click="deleteVendor(vendor.id)"
                                                        variant="destructive" size="sm">
                                                        <Icon name="trash" class="h-4 w-4" />
                                                    </Button>
                                                    <Button v-if="vendor.deleted_at"
                                                        @click="forceDeleteVendor(vendor.id)" variant="destructive"
                                                        size="sm">
                                                        <Icon name="x" class="h-4 w-4" />
                                                    </Button>
                                                </div>
                    ...
                    <DialogFooter>
                        <Button @click="showVendorsModal = false" variant="outline"
                            class="w-full sm:w-auto">Close</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog v-model:open="showDeleteSelectedModal">
                <DialogContent class="max-w-[95vw] sm:max-w-[425px]">
                    <DialogHeader>
                        ...
                        <DialogDescription class="text-sm sm:text-base">
                            Are you sure you want to delete {{ selectedRepairOrders.length }} repair orders? This action
                            cannot be undone.
                        </DialogDescription>
                    </DialogHeader>
            ...
            <Dialog v-model:open="showWoStatusesModal">
                <DialogContent class="max-w-[95vw] sm:max-w-[90vw] md:max-w-[600px]">
                    <DialogHeader>
                        ...
                        <DialogDescription class="text-sm sm:text-base"> Add or remove work order statuses for repair
                            orders. </DialogDescription>
                    </DialogHeader>
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        <TableRow v-if="woStatuses.length === 0">
                                            <TableCell colspan="2" class="py-4 text-center text-sm">No work order
                                                statuses found.</TableCell>
                                        </TableRow>
                                        <TableRow v-for="status in woStatuses" :key="status.id">
                                            <TableCell class="text-xs sm:text-sm">
                                                {{ status.name }}
                                                <span v-if="status.deleted_at"
                                                    class="ml-1 text-xs text-red-500">(Deleted)</span>
                                            </TableCell>
                                            <TableCell>
                                                <div class="flex space-x-1 sm:space-x-2">
                                                    <Button v-if="status.deleted_at" @click="restoreWoStatus(status.id)"
                                                        variant="outline" size="sm">
                                                        <Icon name="undo" class="h-4 w-4" />
                                                    </Button>
                                                    <Button v-if="!status.deleted_at" @click="deleteWoStatus(status.id)"
                                                        variant="destructive" size="sm">
                                                        <Icon name="trash" class="h-4 w-4" />
                                                    </Button>
                                                    <Button v-if="status.deleted_at"
                                                        @click="forceDeleteWoStatus(status.id)" variant="destructive"
                                                        size="sm">
                                                        <Icon name="x" class="h-4 w-4" />
                                                    </Button>
                                                </div>
                    ...
                    <DialogFooter>
                        <Button @click="showWoStatusesModal = false" variant="outline"
                            class="w-full sm:w-auto">Close</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        ...
        <Dialog v-model:open="showCanceledQSInvoicesDialog">
            <DialogContent class="max-h-[90vh] max-w-[95vw] overflow-y-auto md:max-w-[80vw] lg:max-w-4xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        These invoices have a canceled WO status but are still marked as on QS. Please review and take
                        appropriate action.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div class="max-h-[60vh] overflow-x-auto overflow-y-auto">
                    <Table v-if="props.canceledQSInvoices?.length">
                        ...
                        <TableBody>
                            <TableRow v-for="invoice in props.canceledQSInvoices" :key="invoice.ro_number"
                ...
                <DialogFooter class="mt-4 flex flex-col gap-2 sm:flex-row">
                    <Button @click="showCanceledQSInvoicesDialog = false" variant="outline"
                        class="w-full sm:w-auto">Close</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
...
const outstandingDatePicker = ref(null);
const roOpenDatePicker = ref(null);
const roCloseDatePicker = ref(null);
const qsInvoiceDatePicker = ref(null);
...
const openCreateModal = () => {
    ...
    form.on_qs = false;
    form.disputed = false;
    form.dispute_outcome = '';
    form.area_of_concerns = [];

    roOpenDatePicker.value = form.ro_open_date ? parseDate(form.ro_open_date) : null;
    roCloseDatePicker.value = form.ro_close_date ? parseDate(form.ro_close_date) : null;
    qsInvoiceDatePicker.value = form.qs_invoice_date ? parseDate(form.qs_invoice_date) : null;
...
if (outstandingDate.value) {
    outstandingDatePicker.value = parseDate(outstandingDate.value);
}
