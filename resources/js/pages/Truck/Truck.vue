                    ...
                    <div class="relative">
                        ...
                        <div v-if="showUploadOptions" class="absolute right-0 z-10 mt-1 w-48 rounded-md border bg-background shadow-lg">
                            <div class="py-1">
                                ...
                                <a :href="templateUrl" download="Trucks Template.csv" class="block px-4 py-2 text-sm hover:bg-muted">
                                    Download Template
                                </a>
                            </div>
                        </div>
            ...
            <Dialog v-model:open="showDeleteSelectedModal">
                <DialogContent>
                    ...
                    <DialogFooter class="mt-4">
                        ...
                        <Button type="button" @click="deleteSelectedTrucks()" variant="destructive"> Delete Selected </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
...
function changePerPage() {
    const routeName = props.SuperAdmin ? route('truck.index.admin') : route('truck.index', { tenantSlug: props.tenantSlug });
...
const filteredEntries = computed(() => {
    ...
    if (filters.value.search) {
        ...
        result = result.filter((item) => String(item.truckid).includes(searchTerm) || item.vin?.toLowerCase().includes(searchTerm));
    }

    // Apply type filter
    if (filters.value.type) {
        result = result.filter((item) => item.type?.toLowerCase() === filters.value.type.toLowerCase());
    }

    // Apply make filter
    if (filters.value.make) {
        result = result.filter((item) => item.make?.toLowerCase() === filters.value.make.toLowerCase());
    }
...
function submitForm() {
    ...
    if (form.id) {
        form.put(props.SuperAdmin ? route('truck.update.admin', [form.id]) : route('truck.update', [props.tenantSlug, form.id]), {
            data: payload,
            onSuccess: () => {
        ...
        form.post(props.SuperAdmin ? route('truck.store.admin') : route('truck.store', props.tenantSlug), {
            data: payload,
            onSuccess: () => {
...
function confirmDelete() {
    deleteForm.delete(
        props.SuperAdmin ? route('truck.destroy.admin', [truckToDelete.value]) : route('truck.destroy', [props.tenantSlug, truckToDelete.value]),
        {
            onSuccess: () => {
...
function handleImport(e) {
    ...
    importForm.post(props.SuperAdmin ? route('truck.import.admin') : route('truck.import', props.tenantSlug), {
        forceFormData: true,
...
function exportCSV() {
    ...
    const routeName = props.SuperAdmin ? route('truck.export.admin') : route('truck.export', props.tenantSlug);
...
const isAllSelected = computed(() => {
    return filteredEntries.value.length > 0 && filteredEntries.value.every((truck) => selectedTrucks.value.includes(truck.id));
});
