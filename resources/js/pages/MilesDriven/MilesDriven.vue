                    ...
                    <div class="flex flex-col items-center md:items-start gap-2">
                        ...
                        <div class="flex flex-wrap gap-1 md:gap-2">
                            <Button @click="selectDateFilter('yesterday')" variant="outline" size="sm"
                                :class="{ 'border-primary bg-primary/10 text-primary': activeTab === 'yesterday' }">Yesterday</Button>
                            <Button @click="selectDateFilter('current-week')" variant="outline" size="sm"
                                :class="{ 'border-primary bg-primary/10 text-primary': activeTab === 'current-week' }">WTD</Button>
                            <Button @click="selectDateFilter('6w')" variant="outline" size="sm"
                                :class="{ 'border-primary bg-primary/10 text-primary': activeTab === '6w' }">T6W</Button>
                            <Button @click="selectDateFilter('quarterly')" variant="outline" size="sm"
                                :class="{ 'border-primary bg-primary/10 text-primary': activeTab === 'quarterly' }">Quarterly</Button>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-1 md:gap-4 w-full">
                            <div>
                                ...
                                <Input class="py-1 px-1  md:px-2 md:py-1 h-9 lg:px-3 lg:py-2 lg:h-10" id="search"
                                    v-model="filters.search" type="text" placeholder="Search..."
                                    @input="applyFilters" />
                            </div>
                            <div>
                ...
                <CardContent class="p-0">
                        ...
                        <Table class="relative h-[500px] overflow-auto">
                                ...
                                <TableRow class="sticky top-0 z-10 border-b bg-background hover:bg-background">
                                    ...
                                    <TableHead class="cursor-pointer whitespace-nowrap"
                                        ...
                                        <div class="flex items-center">
                                            Week Start Date
                                            <span class="ml-2">
                                                <Icon v-if="sortColumn === 'week_start_date' && sortDirection === 'asc'"
                                                    name="chevron-up" />
                                                <Icon
                                    ...
                                    <TableHead class="cursor-pointer whitespace-nowrap"
                                        ...
                                        <div class="flex items-center">
                                            Week End Date
                                            <span class="ml-2">
                                                <Icon v-if="sortColumn === 'week_end_date' && sortDirection === 'asc'"
                                                    name="chevron-up" />
                                                <Icon
                                    ...
                                    <TableHead class="cursor-pointer whitespace-nowrap" @click="sortBy('miles')">
                                        <div class="flex items-center">
                                            Miles
                                            <span class="ml-2">
                                                <Icon v-if="sortColumn === 'miles' && sortDirection === 'asc'"
                                                    name="chevron-up" />
                                                <Icon v-else-if="sortColumn === 'miles' && sortDirection === 'desc'"
                                                    name="chevron-down" />
                                                <Icon v-else name="chevron-up-down" class="opacity-50" />
                                            </span>
                                        </div>
                            ...
                            <TableBody>
                                ...
                                <TableRow v-if="filteredEntries.length === 0">
                                    <TableCell :colspan="SuperAdmin ? 6 : 5" class="py-4 text-center"> No miles driven
                                        records found </TableCell>
                                </TableRow>
                            </TableBody>
                        </Table>
                    </div>
                    ...
                    <div class="border-t bg-muted/20 px-4 py-3" v-if="entries.links">
                        ...
                        <div class="items-center flex flex-col sm:flex-row justify-between gap-2">
                            <div class=" text-sm text-muted-foreground">Showing {{ filteredEntries.length }} of {{
                                entries.data.length }} entries</div>
                            <!-- responsive here -->
        ...
        <Dialog v-model:open="isModalOpen">
            <DialogContent class="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {{ form.id ? 'Update the miles driven record details.' : `Enter the details for the new miles
                        driven record.` }}
                    </DialogDescription>
                </DialogHeader>
                <form @submit.prevent="submitForm" class="grid gap-4 py-4">
                    ...
                    <div v-if="form.week_start_date" class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-900 dark:text-gray-200">{{ formatDate(form.week_start_date)
                            }}</p>
                        </div>
                        <div>
                            <Label>End (Saturday)</Label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-gray-200">{{ formatDate(form.week_end_date)
                            }}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="notes" v-model="form.notes" placeholder="Optional notes"
                            class="h-24 w-full resize-none rounded-md border border-input bg-background px-3 py-2 text-sm"></textarea>
        ...
        <Dialog v-model:open="isDeleteModalOpen">
            <DialogContent class="w-[95vw] sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription class="text-sm md:text-base"> Are you sure you want to delete this miles driven
                        record? This action cannot be undone. </DialogDescription>
                </DialogHeader>
...
const dateFromPicker = ref(filters.value.dateFrom ? parseDate(filters.value.dateFrom) : null);
const dateToPicker = ref(filters.value.dateTo ? parseDate(filters.value.dateTo) : null);
