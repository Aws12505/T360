  ...
  <Card class="bg-background dark:bg-background border border-muted/30">
    <CardContent class="p-1 md:p-4">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-4">
        ...
        <div
          class="bg-gradient-to-br from-muted/10 to-muted/30 rounded-lg p-4 shadow-md border border-muted/20 hover:shadow-lg transition-shadow"
        >
          <h3 class="text-lg font-semibold text-foreground mb-2">Green Zone Score</h3>
          <div class="text-4xl font-bold" :class="getScoreTextColor(data.greenZoneScore)">
            {{ Math.round(data.greenZoneScore) }}
          </div>
          ...
          <div class="mt-3 pt-3 border-t border-muted/30">
            <div class="flex items-center justify-between">
              ...
              <span class="text-2xl font-bold text-indigo-600">{{
                Math.round(data.infractions?.driverStar || 0)
              }}</span>
            </div>
        ...
        >
          ...
          <div class="space-y-2">
            <div v-if="data.topDrivers && data.topDrivers.length > 0">
              <div
                v-for="(driver, index) in data.topDrivers"
                :key="index"
                class="flex justify-between items-center"
              >
                <span class="text-foreground">{{ driver.name }}</span>
                <span class="font-medium" :class="getScoreTextColor(driver.score)">{{
        ...
        >
          ...
          <div class="space-y-2">
            <div v-if="data.bottomDrivers && data.bottomDrivers.length > 0">
              <div
                v-for="(driver, index) in data.bottomDrivers"
                :key="index"
                class="flex justify-between items-center"
              >
                <span class="text-foreground">{{ driver.name }}</span>
      ...
      <div class="mt-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-2 md:gap-4">
          <div
            v-for="(value, type) in data.alerts"
            :key="type"
      ...
      <div class="mt-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-2 md:gap-4">
          <div
            v-for="(value, type) in filteredVisibleInfractions"
            :key="type"
            class="bg-gradient-to-br from-muted/10 to-muted/30 rounded-lg p-2 md:p-4 text-center shadow-md border border-muted/20 hover:shadow-lg transition-shadow"
          >
            <div class="text-sm text-muted-foreground mb-2">
              {{ formatInfractionType(type) }}
            </div>
        ...
        <div v-if="hasMoreInfractions" class="mt-4 text-center">
          ...
          >
            <Icon :name="showMore ? 'chevron-up' : 'chevron-down'" class="mr-2 h-4 w-4" />
        ...
        <div v-if="showMore" class="mt-4">
          <div class="grid grid-cols-2 md:grid-cols-5 gap-2 md:gap-4">
            <div
              v-for="(value, type) in hiddenInfractions"
              :key="type"
              class="bg-gradient-to-br from-muted/10 to-muted/30 rounded-lg p-2 md:p-4 text-center shadow-md border border-muted/20 hover:shadow-lg transition-shadow"
            >
              <div class="text-sm text-muted-foreground mb-2">
      ...
      <div v-if="activeTab !== 'yesterday'" class="mt-6 text-center">
        <Button
          ...
          class="w-full md:w-auto shadow-sm hover:shadow transition-shadow"
        >
          <Icon :name="showGraphs ? 'eye-off' : 'eye'" class="mr-2 h-4 w-4" />
      ...
      <div v-if="showGraphs" class="mt-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4 lg:gap-6">
          ...
          <div
            ...
            class="bg-gradient-to-br from-muted/10 to-muted/30 rounded-lg p-4 shadow-lg border border-muted/20"
          >
          ...
          <div
            v-if="activeTab !== 'yesterday'"
            :class="{ 'md:col-span-2': activeTab === 'yesterday' }"
            class="bg-gradient-to-br from-muted/10 to-muted/30 rounded-lg p-4 shadow-lg border border-muted/20"
          >
...
const themeEl = () => {
  // Prefer <html> when it has dark, otherwise fallback to <body>
  const html = document.documentElement;
  const body = document.body;

  if (html.classList.contains("dark")) return html;
  if (body?.classList.contains("dark")) return body;

  // Light mode: vars are on :root, so <html> is safest
  return html;
};

const cssHsl = (name) => {
  const raw = getComputedStyle(themeEl()).getPropertyValue(name).trim();
  if (!raw) return null; // var missing => null
  if (raw.startsWith("hsl(")) return raw; // already formatted
  return `hsl(${raw})`; // convert "235 51% 37%" -> "hsl(235 51% 37%)"
};
...
const getChartColors = () => {
  ...
  const colors = vars.map(cssHsl);

  // fallback so it never renders all-grey
  return colors.every(Boolean)
    ? colors
    : ["#061455", "#1911B2", "#678AC6", "#EA0E45", "#FF9898", "#10b981"];
};
...
const initializeCharts = () => {
  ...
  if (lineChartCanvas.value && props.activeTab !== "yesterday") {
    ...
    if (ctx) {
      ...
      const dataValues =
        props.data.lineChartData?.map((item) => item.greenZoneScore) || [];
