<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :tenantSlug="tenantSlug" :permissions="props.permissions">
        ...
        <div class="w-full md:max-w-2xl lg:max-w-3xl xl:max-w-6xl lg:mx-auto m-0 p-2 md:p-4 lg:p-6 space-y-2 md:space-y-4 lg:space-y-6">
            ...
            <div v-if="props.entries.data.length === 0" class="flex flex-col items-center justify-center rounded-lg border bg-muted/20 py-16">
                ...
                <Dialog v-model:open="showModal">
                    <DialogContent class="w-[90vw] sm:max-w-lg md:max-w-2xl lg:max-w-4xl">
                        <DialogHeader>
                            ...
                            <DialogDescription> Fill in the details to {{ formAction.toLowerCase() }} an entry. </DialogDescription>
                        </DialogHeader>
                        ...
                        <form @submit.prevent="submitForm" class="grid max-h-[70vh] gap-6 overflow-y-auto p-1">
                            ...
                            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
                                ...
                                <div
                                    v-for="col in ['driver_name', 'user_name', 'group', 'group_hierarchy', 'vehicle_type']"
                                    :key="col"
                                    class="space-y-1"
                                >
                                ...
                                <div
                                    ...
                                    class="space-y-1"
                                >
                                    <Label :for="col" class="text-sm capitalize">{{ col.replace(/_/g, ' ') }}</Label>
                                    <Input :id="col" v-model="form[col]" :type="getInputType(col)" :step="getStep(col)" :min="getMin(col)" />
                                </div>
                            </div>

                            <DialogFooter class="mt-6 flex-col space-y-2 border-t pt-4 sm:flex-row sm:justify-end sm:space-x-2 sm:space-y-0">
                ...
                <Dialog v-model:open="showDeleteSelectedModal">
                    <DialogContent>
                        ...
                        <DialogFooter class="mt-4">
                            ...
                            <Button type="button" @click="deleteSelectedEntries()" variant="destructive"> Delete Selected </Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
...
function handleImport(e) {
    ...
    const routeName = props.SuperAdmin ? route('safety.import.admin') : route('safety.import', props.tenantSlug);
...
function exportCSV() {
    ...
    const routeName = props.SuperAdmin ? route('safety.export.admin') : route('safety.export', props.tenantSlug);
...
function selectDateFilter(filter) {
    ...
    const routeName = props.tenantSlug ? route('safety.index', { tenantSlug: props.tenantSlug }) : route('safety.index.admin');
