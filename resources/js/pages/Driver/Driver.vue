    ...
    <div
      ...
      <div class="flex flex-col sm:flex-row justify-between items-center px-2 mb-2 md:mb-4 lg:mb-6">
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <Button @click="showImportModal = true" v-if="permissionNames.includes('drivers.import')" variant="secondary"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-1 md:gap-4 w-full">
              <div>
                ...
                <Input class="py-1 px-1 md:px-2 md:py-1 h-9 lg:px-3 lg:py-2 lg:h-10" id="search"
                  v-model="filters.search" type="text" placeholder="Search by name or email..." @input="applyFilters" />
              </div>
            ...
            <Table class="relative h-[600px] overflow-auto">
              ...
              <TableBody>
                ...
                <TableRow v-for="driver in filteredEntries" :key="driver.id">
                  <!-- Row checkbox -->
                  <TableCell class="text-center" v-if="permissionNames.includes('drivers.delete')">
                    <input type="checkbox" :value="driver.id" v-model="selectedDrivers"
                      class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
                  </TableCell>
                  ...
                  <TableCell v-for="col in tableColumns" :key="col">
                    <template v-if="col === 'hiring_date'">
                      {{ driver[col] ? formatDate(driver[col]) : "—" }}
                    </template>
                  ...
                  ">
                    <div class="flex space-x-2">
                      <Link v-if="permissionNames.includes('drivers.profile.view')" :href="viewUrl(driver)"
        ...
        <DialogContent class="sm:max-w-4xl max-w-[95vw] mx-auto p-2 md:p-4 lg:p-6">
          ...
          <div class="max-h-[80vh] overflow-y-auto pr-2">
            <form @submit.prevent="submitForm" class="grid grid-cols-1 gap-2 md:gap-4 sm:grid-cols-2 mt-2 md:mt-4">
              ...
              <!-- Footer buttons -->
              <DialogFooter class="col-span-2 mt-4 flex flex-col sm:flex-row gap-2 sm:gap-4 justify-end">
...
const filteredEntries = computed(() => {
  ...
  if (filters.value.dateTo) {
    result = result.filter(
      (d) => !d.hiring_date || d.hiring_date <= filters.value.dateTo
    );
  }
...
function submitForm() {
  ...
  Object.keys(form.data()).forEach((key) => {
    ...
    if (key === "hiring_date") {
      fd.append("hiring_date", form.hiring_date || "");
      return;
    }
