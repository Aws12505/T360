      ...
      <div class="w-full md:max-w-xl lg:max-w-2xl xl:max-w-6xl m-auto pt-2 space-y-2 md:space-y-4 lg:space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          ...
          <div class="flex flex-wrap gap-3">
            ...
            <Button @click="showImportModal = true" v-if="permissionNames.includes('performance.import')"
              variant="secondary" class="px-2 py-0 md:px-4 md:py-2 shadow-sm hover:shadow transition-all">
              ...
              <TableBody>
                ...
                <TableRow v-for="item in filteredPerformances" :key="item.id" class="hover:bg-muted/50">
                  <TableCell class="text-center" v-if="permissionNames.includes('performance.delete')">
                    <input type="checkbox" :value="item.id" v-model="selectedPerformances"
                      class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
                  </TableCell>
                  ...
                  <TableCell class="min-w-[120px]" v-if="
                    permissionNames.includes('performance.update') ||
                    permissionNames.includes('performance.delete')
                  ">
          ...
          <div class="border-t bg-muted/20 px-4 py-3" v-if="performances.links">
            <div class="flex flex-col sm:flex-row justify-between items-center gap-2">
              <div class="flex items-center gap-4 text-sm text-muted-foreground">
                <span>Showing {{ filteredPerformances.length }} of
              ...
              <div class="flex flex-wrap">
                <Button v-for="link in performances.links" :key="link.label" @click="visitPage(link.url)"
        ...
        <DialogContent class="max-w-[95vw] sm:max-w-[90vw] md:max-w-4xl">
          ...
          <form @submit.prevent="submitForm"
            ...
            <div v-if="SuperAdmin" class="col-span-2">
              ...
              <div class="relative mt-1">
                ...
                <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2 text-gray-700">
                  <svg class="h-3 w-3 opacity-50 sm:h-4 sm:w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                    fill="currentColor">
            ...
            <div class="col-span-2 sm:col-span-1">
              ...
              <Popover v-model:open="performanceDateOpen">
                  ...
                  <Button id="date" variant="outline"
                    ...
                    {{
                      performanceDatePicker
                        ? df.format(performanceDatePicker.toDate(getLocalTimeZone()))
                        : 'Pick a date'
                    }}
                  </Button>
                </PopoverTrigger>
            ...
            <div class="col-span-2 sm:col-span-1">
              ...
              <Input id="on_time_to_destination" v-model="form.on_time_to_destination" type="number" step="0.01"
                required class="h-9 px-3 py-1 text-xs sm:h-10 sm:text-sm" />
            </div>

            <!-- Maintenance Variance to Spend Field -->
            <div class="col-span-2 sm:col-span-1">
              <Label for="maintenance_variance_to_spend" class="text-xs sm:text-sm">Maintenance Variance to
                Spend</Label>
              <Input id="maintenance_variance_to_spend" v-model="form.maintenance_variance_to_spend" type="number"
                step="0.001" required class="h-9 px-3 py-1 text-xs sm:h-10 sm:text-sm" />
            </div>
            ...
            <div class="col-span-2 flex items-center gap-2">
              <input id="meets_safety_bonus_criteria" v-model="form.meets_safety_bonus_criteria" type="checkbox"
              ...
              <div class="space-y-4">
                ...
                <div
                  ...
                  <label class="cursor-pointer mt-3">
                    ...
                    <input ref="importFileInput" type="file" class="hidden" @change="onImportInputChange"
                      accept=".csv,text/csv" :disabled="isValidating" />
                  </label>
                        ...
                        <TableRow v-for="row in importValidationResults.invalid" :key="row.rowNumber"
                          ...
                          <TableCell class="text-sm text-muted-foreground">
                            <div class="flex flex-wrap gap-x-3 gap-y-1">
                              ...
                              <span v-if="!row.preview?.length" class="italic text-muted-foreground">
                                —
                              </span>
                            </div>
                          </TableCell>
...
const page = usePage();
const performanceDatePicker = ref(null);
const performanceDateOpen = ref(false);
...
const handlePerformanceDateSelect = (val) => {
  performanceDatePicker.value = val ?? null;
  form.date = val
    ? val.toDate(getLocalTimeZone()).toISOString().split("T")[0]
    : "";
  performanceDateOpen.value = false;
};
/** Drag state */
...
function openCreateModal() {
  ...
  performanceDatePicker.value = form.date ? parseDate(form.date) : null;
