...
class DriverDataService
{
     ...
     *  - traffic_light_violation : total traffic light violations
     *  - speeding_violations     : total speeding violations
     *  - following_distance      : total following distance violations
     *  - roadside_parking        : total roadside parking violations
     *  - driver_distraction      : total driver distraction violations
     *  - sign_violations         : total sign violations
     *
    ...
    public function getDriversOverallPerformanceCoaching(Carbon $startDate, Carbon $endDate): array
    {
        ...
        "), DB::raw("CONCAT(drivers.first_name,' ',drivers.last_name)"), '=', 'del.driver_name')
            ...
            ->leftJoin(DB::raw("
                        (
                            SELECT
                                ...
                                COALESCE(SUM(sd.traffic_light_violation), 0) AS traffic_light_violation,
                                COALESCE(SUM(sd.speeding_violations), 0) AS speeding_violations,
                                COALESCE(SUM(sd.following_distance), 0) AS following_distance,
                                COALESCE(SUM(sd.roadside_parking), 0) AS roadside_parking,
                                COALESCE(SUM(sd.driver_distraction), 0) AS driver_distraction,
                                COALESCE(SUM(sd.sign_violations), 0) AS sign_violations,
                                COALESCE(SUM(
            ...
            ->select([
                ...
                DB::raw("COALESCE(safe.traffic_light_violation, 0) AS traffic_light_violation"),
                DB::raw("COALESCE(safe.speeding_violations, 0) AS speeding_violations"),
                DB::raw("COALESCE(safe.following_distance, 0) AS following_distance"),
                DB::raw("COALESCE(safe.roadside_parking, 0) AS roadside_parking"),
                DB::raw("COALESCE(safe.driver_distraction, 0) AS driver_distraction"),
                DB::raw("COALESCE(safe.sign_violations, 0) AS sign_violations"),
            ])
            ->get()
        ...
        foreach ($rows as $r) {
            ...
            $out[] = [
                ...
                'traffic_light_violation' => (int) $r->traffic_light_violation,
                'speeding_violations' => (int) $r->speeding_violations,
                'following_distance' => (int) $r->following_distance,
                'roadside_parking' => (int) $r->roadside_parking,
                'driver_distraction' => (int) $r->driver_distraction,
